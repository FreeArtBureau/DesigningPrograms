...
public class GeoLetters_05 extends PApplet {
...
public void draw() {
  ...
  // First letter
  noStroke();
  float freqCol = sin(frameCount*0.030f) * 230;
  float freqCol2 = sin(frameCount*0.015f) * 50;
  float freqCol3 = sin(frameCount*0.030f) * 100;
  ...
  float freq = sin(pow(0.001f, 0.15f)) * 0.0130f;
  //float freq = sin(frameCount * 0.005) * 0.015;
  A.dia = sin(frameCount*freq) * 1.7f;
  A.xAxisWaves(false, 1, 1,1, .015f, 30);
...
class Letter {
  ...
  public void display() {
    ...
    for (int i=0; i<myPoints.length; i++) {
      pushMatrix();
      translate(loc[i].x+xOff, loc[i].y+yOff);
      ellipse(0, 0, dia, dia);
      popMatrix();
    }
    popMatrix();
  ...
  ///////////////////////////////////////
  public void xOscillate(float freq, float amp){
    /*
    for (int i=0; i<myPoints.length; i++) {
    float offX = noise(frameCount+(loc[i].x * freq), frameCount+(loc[i].y * freq)) * 300;
    xOff += cos(offX) * amp;
    //yOff += sin(offX) * amp;
  }
  */
    xOff += cos(frameCount * freq) * amp;
  }
   ...
   * @param orientation -1/+1 for right/left movement
   ...
   */
  public void xAxisWaves(boolean spin, int direction, int orientation, int angleFact, float freq, float amp) {
    this.dir = direction;
    if(orientation==1){
  ...
  static public void main(String[] passedArgs) {
    String[] appletArgs = new String[] { "GeoLetters_05" };
    if (passedArgs != null) {
