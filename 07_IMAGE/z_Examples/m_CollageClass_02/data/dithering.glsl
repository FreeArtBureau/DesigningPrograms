...
uniform float ditherGrayscaleFact;
uniform float ditherGrayscaleMult;
uniform float fragCoordXY;
uniform float pixelFrag;
...
void main(void) {
	...
	float grayscale = length(sourcePixel*vec3(0.2126,0.7152,0.072));
	
	vec3 ditherPixel = texture2D(noiseTexture, vec2(mod(gl_FragCoord.xy/vec2(fragCoordXY,fragCoordXY),pixelFrag))).xyz;
	float ditherGrayscale = (ditherPixel.x + ditherPixel.y + ditherPixel.z) / ditherGrayscaleFact;
	ditherGrayscale -= ditherGrayscaleMult;
